        ...
        if (!target) {
            return message.reply({
                embeds: [
                    new EmbedBuilder()
                        .setColor("#a18cd1")
                        .setAuthor({ name: "Command: vmute", iconURL: message.author.displayAvatarURL({ dynamic: true }) })
                        .setDescription("Mute a user in a voice channel.")
                        .addFields(
                            { name: "Usage:", value: "- &vmute \`userid\`", inline: false },
                            { name: "Example:", value: `- &vmute <@&${message.author.id}>\n- &vmute \`${message.author.id}\``, inline: false }
                        )
                ]
            });
        }
        ...
        if (target.voice.serverMute) {
            return message.reply({
                embeds: [
                    new EmbedBuilder()
                        .setColor("#a18cd1")
                        .setDescription(`<:info:1402098933261533285> ${target.user.tag} is already muted.`)
                ]
            });
        }
        ...
        try {
            ...
            return message.reply({
                embeds: [
                    new EmbedBuilder()
                        .setColor("#a18cd1")
                        .setDescription(`<:yes:1402096123434500096> Successfully muted **${target.user.tag}** in the voice channel.`)
                ]
            });
        } catch (error) {
            ...
            return message.reply({
                embeds: [
                    new EmbedBuilder()
                        .setColor("#a18cd1")
                        .setDescription("<:info:1402098933261533285> An error occurred while trying to mute the user.")
                ]
            });
        }
    }
